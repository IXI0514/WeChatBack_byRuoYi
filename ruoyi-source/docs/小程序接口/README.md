...
`POST /api/token`：仅使用白名单 miniappId 获取凭证，不验证调用方身份；也可继续使用 AES 密钥本地生成。有效期由系统参数 `miniapp.token.expire.minutes`（分钟）统一控制，未配置/留空不过期。中继台查询及登录验证均需传 token；领取入口除外。配置及示例见 [22 · 接口说明](./接口说明.md)。
